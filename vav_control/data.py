...
vav_flow_uuid = [
    "bdc5d778-a18c-581c-8a71-55579b19cb3d",
...
vav_setpt_uuid = [
    "3f9c81bc-16eb-5065-a630-1a288c60c466",  # 1
...
def data_acquisition():
    ...
    fan_power_total_data = client.data_uuid(
        [fan_power_total_uuid1], start, end, cache=true
    )[0]
    ...
    for j in range(min([len(x) for x in setpt_download])):
        value = sum(
            [setpt_download[i][j][1] for i in range(len(setpt_download))]
        ) / len(setpt_download)
    ...
    for i in range(
        ...
        power = (
            fan_power_total_data[i][1]
            * flow_floor_4[i][1]
            / (air_a_flow_data[i][1] + air_b_flow_data[i][1])
        )
    ...
    for i in range(
        ...
        result = (
            (
                capacity
                * (air_a_sat_data[i][1] - air_a_mat_data[i][1])
                * air_a_flow_data[i][1]
            )
            + (
                capacity
                * (air_b_sat_data[i][1] - air_b_mat_data[i][1])
                * air_b_flow_data[i][1]
            )
        ) * ratio[i][1]
    ...
    for i in range(min(len(light_data), len(recep_data), len(cool_power_data))):
        h_c_power_data.append(
            [
                cool_power_data[i][0],
                cool_power_data[i][1] + light_data[i][1] + recep_data[i][1],
            ]
        )
    ...
    t_data_sychro_average = [
        (a + b) / 2 for a, b in zip(t_data1_synchro, t_data2_synchro)
    ]
    for i in range(len(t_data_sychro_average)):
        t_data_sychro_average[i] = (t_data_sychro_average[i] - 32) * 5 / 9
    hum_data_sychro_average = [
        (a + b) / 2 for a, b in zip(hum_data3_synchro, hum_data4_synchro)
    ]
    co2_data_sychro_average = [
        (a + b) / 2 for a, b in zip(co2_data5_synchro, co2_data6_synchro)
    ]
    ...
    for i in imposed_time:
        ...
        if cal_day == 2 and (hour < 16 and hour >= 10):
            ...
            if cal_day == 2 and (
                (hour < 10 and hour >= 7) or (hour < 19 and hour >= 16)
            ):
                number = 15
            else:
                number = 0
    ...
    data_list = {
        "timestamp": imposed_time,
        "temperature": t_data_sychro_average,
        "humidity": hum_data_sychro_average,
        "calendar data": calendar_data,
        "human date": human_date,
        "season": season,
        "co2": co2_data_sychro_average,
        "outdoor temperature": t_outdoor_data7_synchro,
        "ventilation": vent_power_data_synchro,
        "h/c power": h_c_power_data_synchro,
        "setpoint temperature": t_setpt_data_synchro,
...
def real_time_data(client):
    ...
    for uuid in vav_setpt_uuid:
        s = (
            s
            + (client.latest("uuid = '" + uuid + "'")[0]["readings"][0][1] - 32) * 5 / 9
        )
    ...
    # outdoor t
    t_outdoor = (
        (client.latest("uuid = '" + t_outdoor_uuid7 + "'")[0]["readings"][0][1] - 32)
        * 5
        / 9
    )
    ...
    fan_power_total_data = client.latest("uuid = '" + fan_power_total_uuid1 + "'")[0][
        "readings"
    ][0][1]

    air_a_sat_data = (
        (client.latest("uuid = '" + air_a_sat_uuid1 + "'")[0]["readings"][0][1] - 32)
        * 5
        / 9
    )
    air_b_sat_data = (
        (client.latest("uuid = '" + air_b_sat_uuid1 + "'")[0]["readings"][0][1] - 32)
        * 5
        / 9
    )
    air_a_mat_data = (
        (client.latest("uuid = '" + air_a_mat_uuid1 + "'")[0]["readings"][0][1] - 32)
        * 5
        / 9
    )
    air_b_mat_data = (
        (client.latest("uuid = '" + air_b_mat_uuid1 + "'")[0]["readings"][0][1] - 32)
        * 5
        / 9
    )

    air_a_flow_data = client.latest("uuid = '" + air_a_flow_uuid1 + "'")[0]["readings"][
        0
    ][1]
    air_b_flow_data = client.latest("uuid = '" + air_b_flow_uuid1 + "'")[0]["readings"][
        0
    ][1]
    flow_floor_4 = 0
    for uuid in vav_flow_uuid:
        flow_floor_4 = (
            flow_floor_4 + client.latest("uuid = '" + uuid + "'")[0]["readings"][0][1]
        )

    vent_power = (
        fan_power_total_data * flow_floor_4 / (air_a_flow_data + air_b_flow_data)
    )

    capacity = 0.00056937  # kw/c.cfm
    ratio = flow_floor_4 / (air_a_flow_data + air_b_flow_data)
    cool_power = (
        (capacity * (air_a_sat_data - air_a_mat_data) * air_a_flow_data)
        + (capacity * (air_b_sat_data - air_b_mat_data) * air_b_flow_data)
    ) * ratio
