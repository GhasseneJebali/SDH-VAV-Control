...
def Data_Validation(data, X):
    ...
    if (X[1] < lower["H/C power"] * 0.75) or (X[1] > upper["H/C power"] * 1.25):
        valid = 0
    if (X[2] < lower["Setpoint Temperature"] * 0.75) or (
        X[2] > upper["Setpoint Temperature"] * 1.25
    ):
        valid = 0
    if (X[3] < lower["Outdoor Temperature"] * 0.75) or (
        X[3] > upper["Outdoor Temperature"] * 1.25
    ):
        valid = 0
    ...
    for i in range(len(data["Temperature"]) - int(Prediction_horizon / 15)):
        X.append(
            [
                data["Temperature"][i],
                data["H/C power"][i],
                data["Setpoint Temperature"][i],
                data["Outdoor Temperature"][i],
                data["Calendar data"][i],
                data["Human_power"][i],
            ]
        )
